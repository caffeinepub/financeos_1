...
import { RiskProfile } from '../backend';
import { PieChart as RechartsPie, Pie, Cell, ResponsiveContainer, Legend, Tooltip, LineChart, Line, XAxis, YAxis, CartesianGrid } from 'recharts';
...
export function ModelPortfolioTab() {
  ...
  const [portfolioType, setPortfolioType] = useState<'mutualFunds' | 'etf' | 'both'>('both');
  const [initialCapital, setInitialCapital] = useState<string>('100000');
  const [investmentType, setInvestmentType] = useState<'sip' | 'lumpsum' | null>(null);
  const [sipAmount, setSipAmount] = useState<string>('5000');
  const [allocations, setAllocations] = useState<AssetAllocation[]>([]);
  const [forecastData, setForecastData] = useState<Array<{ year: number; value: number }>>([]);

  const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884D8', '#82CA9D', '#FFC658', '#FF6B9D'];

  // Auto-generate portfolio when inputs change
  useEffect(() => {
    generateModelPortfolio();
  }, [riskProfile, portfolioType, initialCapital]);

  // Auto-generate forecast when investment type or amounts change
  useEffect(() => {
  ...
  const generateModelPortfolio = () => {
    // Fund-category-based logic only (no company names)
    const assets: AssetAllocation[] = [];
    
    if (portfolioType === 'mutualFunds' || portfolioType === 'both') {
      // Fund categories based on risk profile
  ...
  const generateForecast = () => {
    ...
    const expectedReturn = riskProfile === RiskProfile.conservative ? 0.08 : 
                          riskProfile === RiskProfile.moderate ? 0.12 : 0.15;

    const data: Array<{ year: number; value: number }> = [];

    if (investmentType === 'lumpsum') {
      for (let year = 0; year <= 25; year++) {
        const value = capital * Math.pow(1 + expectedReturn, year);
        data.push({ year, value });
      }
    } else if (investmentType === 'sip') {
      for (let year = 0; year <= 25; year++) {
        // FV = P(1+r)^n + PMT × [((1+r)^n - 1) / r]
        const lumpsumValue = capital * Math.pow(1 + expectedReturn, year);
        const sipValue = sip * 12 * ((Math.pow(1 + expectedReturn, year) - 1) / expectedReturn);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure your investment preferences - portfolio updates automatically
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label htmlFor="riskProfile">Investor Risk Profile</Label>
              <Select value={riskProfile} onValueChange={(value) => setRiskProfile(value as RiskProfile)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={RiskProfile.conservative}>Conservative</SelectItem>
            ...
            <div className="space-y-2">
              ...
              <Select value={portfolioType} onValueChange={(value) => setPortfolioType(value as 'mutualFunds' | 'etf' | 'both')}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              <Label htmlFor="initialCapital">Base Initial Capital (₹)</Label>
              <Input
                id="initialCapital"
                type="number"
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      <RechartsPie>
                        <Pie
                          ...
                          cy="50%"
                          labelLine={false}
                          label={({ name, allocation }) => `${name.split(' ')[0]}: ${allocation.toFixed(1)}%`}
                          outerRadius={80}
                          fill="#8884d8"
                          dataKey="allocation"
                        >
                      ...
                      {allocations.map((asset, index) => (
                        <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                          ...
                          <div className="text-right">
                            <p className="text-sm font-semibold">{asset.allocation.toFixed(2)}%</p>
                            <p className="text-xs text-muted-foreground">
                              {formatCurrency((parseFloat(initialCapital) || 0) * asset.allocation / 100)}
                            </p>
                          </div>
                        </div>
                      ))}
              ...
              {forecastData.length > 0 && (
                <Card className="mt-6">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Projected portfolio value over 25 years ({investmentType === 'sip' ? 'SIP' : 'Lumpsum'} investment)
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                        ...
                        <CardContent className="pt-6">
                          <div className="text-center space-y-1">
                            ...
                            <p className="text-xl font-bold text-blue-600">
                              {formatCurrency(parseFloat(initialCapital) || 0)}
                            </p>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card className="bg-green-500/5 border-green-500/20">
                        <CardContent className="pt-6">
                          <div className="text-center space-y-1">
                            <p className="text-xs text-muted-foreground">Projected Value (25Y)</p>
                            <p className="text-xl font-bold text-green-600">
                              {formatCurrency(forecastData[forecastData.length - 1]?.value || 0)}
                            </p>
                          </div>
                        </CardContent>
                      </Card>
                        ...
                        <CardContent className="pt-6">
                          <div className="text-center space-y-1">
                            ...
                            <p className="text-xl font-bold text-primary">
                              {riskProfile === RiskProfile.conservative ? '8%' : 
                               riskProfile === RiskProfile.moderate ? '12%' : '15%'} p.a.
                            </p>
                          </div>
                        </CardContent>
                      </Card>
